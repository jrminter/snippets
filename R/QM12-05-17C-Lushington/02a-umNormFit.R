...
# to the nornally binned data
# to get a single mode normal fit
# 
...
do.the.size.plot = function(){
  ...
  mu.x <- c(c,c)
  mu.y <- c(0, max(y))
  lines(mu.x,mu.y, col="blue", lwd=2)
  ...
  mtext(paste("Normal size distribution of",
              str.sample),
        side=3, line=1, cex=t.cex)
  size.text <- paste('ECD (mu =',
                      round(c, 1), str.unit)
  size.text <- paste( size.text, ', sd = ',
                      round(b, 1),
					  ', se = ', round(s.e, 4),
                      ', n = ', n.part,
                      ')', sep='')

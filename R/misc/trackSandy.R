...
str.wd <-'~/Dropbox/work/_snippets/R/misc/'
setwd(str.wd)
...
plot.sandy <- function(){
  ...
  map("state", boundary = FALSE, col="gray",
      add = TRUE,xlim=xlim)

  # plot the track with triangles and colors
  lines(x=wx$Longitude,y=wx$Latitude,
        col="black",cex=0.75)
  points(x=wx$Longitude,y=wx$Latitude,
         col=wx$color,pch=17,cex=0.75)

  # annotate it with the current & projects
  # strength status + forecast
  text(x=wx$Longitude,y=wx$Latitude,
       col='blue',labels=wx$Status,adj=c(-.15),cex=0.33)

  # get the forecast "cone" from the KML that google's
  # crisis map provides
  # NOTE: you need curl & unzip (with funzip) on your system
  # NOTE: Google didn't uniquely name the cone they provide, 
  # so this will break post-Sandy. I suggest saving the last
  # KML & track files out out if you want to save this for
  # posterity

  # this gets the cone and makes it into a string that
  # getKMLcoordinates can process
  ...
  polygon(coords$X1,coords$X2, pch=16, border="red",
          col='#FF000033',cex=0.25)
...
str.pdf <- './trackSandy.pdf'

pdf.options(useDingbats=TRUE)
pdf(file="./temp.pdf", width=6, height=6, pointsize=8)
plot.sandy()
